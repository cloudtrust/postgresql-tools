...
class TestcriptPSQLScriptRun():
    ...
    def test_missing_arguments(self):
        ...
        with pytest.raises(Exception):
            exec_script = python3.bake(script_name, "--sql-scripts", "test_create_user.sql", "test_drop_user.sql",
        ...
        with pytest.raises(Exception):
            ...
            exec_script()
        with pytest.raises(Exception):
            exec_script = python3.bake(script_name, "--sql-scripts", "test_create_user.sql",
                                       "--database-server-ip", "127.0.0.1",
                                       "--database-config-file", "./test_config/psql.json")
            exec_script()
    ...
    def test_inexistent_sql_file(self):
        ...
        with pytest.raises(Exception):
            exec_script = python3.bake(script_name, "--sql-scripts", "no_existent_file.sql", "no_existent_file.sql",
    ...
    def test_invalid_ip(self):
        ...
        with pytest.raises(sh.ErrorReturnCode_1):
            exec_script = python3.bake(script_name, "--sql-scripts", "test_create_user.sql", "test_drop_user.sql", "--database-server-ip", "127.0.1",
                                   "--database-config-file", "./test_config/psql.json")
            val = exec_script()
    ...
    def test_script_working(self):
        ...
        script_name = "postgresql_execute_script.py"
        create_user = python3.bake(script_name, "--sql-scripts", "test_create_user.sql", "test_drop_user.sql", "--database-server-ip", "127.0.0.1",
                                   "--database-config-file", "./test_config/psql.json")
        create_user()

        drop_user = python3.bake(script_name, "--sql-scripts", "test_drop_user.sql", "test_create_user.sql", "--database-server-ip", "127.0.0.1",
                                   "--database-config-file", "./test_config/psql.json")
        drop_user()
